...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        if self.direction == 1:
            if self.place.entrance != colony.hive and self.armor > 0:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        p = self.place
        while p.entrance != hive and p.bees == []:
            p = p.entrance
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_imposter:
            ...
            while p:
                if p.ant:
                    if hasattr(p.ant, 'damage') and p.ant.is_buffed == False:
                        p.ant.damage *= 2
                        p.ant.is_buffed = True
                    if p.ant.is_container:
                        contained_ant = p.ant.contained_ant
                        if contained_ant and hasattr(contained_ant, 'damage') and contained_ant.is_buffed == False:
                            contained_ant.damage *= 2
                            contained_ant.is_buffed = True                        
                p = p.exit
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    if effect == 'make_slow':
        ...
        bee.slow_time += duration
    elif effect == 'make_scare':
        if bee.is_scared == False:
            bee.is_scared = True
            bee.scare_time = duration
            bee.action = effect(bee.action, bee)      
    # END Problem EC
...
class LaserAnt(ThrowerAnt):
        ...
        while p != hive:
            if p.ant:
                ...
                if p.ant.is_container and p.ant.contained_ant:
                    dic[p.ant.contained_ant] = distance
            if p.bees != []:
                for bee in p.bees[:]:
                    dic[bee] = distance
            distance += 1
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        final_damage = self.damage - self.insects_shot * 0.05 - distance * 0.2
        if final_damage > 0:
            return final_damage
        else:
            return 0 
