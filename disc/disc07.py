...
run_docstring_examples(remove_duplicates, globals(), True)
#%%
class A():
    def __init__(self, x):
        self.x = x
...
def average(t):
    ...
    helper(t)
    return total / count
run_docstring_examples(average, globals(), True)
#%%
def long_paths(t, n):
    ...
    if n <= 0 and t.is_leaf():
        lst.append(Link(t.label))
    for b in t.branches:
        for e in long_paths(b, n-1):
            lst.append(Link(t.label, e))
    return lst
run_docstring_examples(long_paths, globals(), True)
