    ...
    if any(lst):
        ...
        for i in range(len(lst)):
            if i == 0:
                ...
                new_lst = lst[:i-1] + lst[i+2:]
            value_lst += [lst[i] * max_product(new_lst)]
...
def redundant_map(t, f):
    ...
    new_b = [redundant_map(b, new_f) for b in branches(t)]
    return tree(new_label, new_b)
run_docstring_examples(redundant_map, globals(), True)
