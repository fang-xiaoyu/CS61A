...
run_docstring_examples(taxicab, globals(), True)
#%%
def flatten(lst):
...
#%%
t = tree(1,[tree(1,[tree(1),tree(3)]), tree(1)])
print_tree(t)
print(t)
#%%
# Mobiles
...
def balanced(m):
    ...
    if is_weight(end_l) and is_weight(end_r):
        return length(side_l) * size(end_l) == length(side_r) * size(end_r)
    else:
        weight_l, weight_r = total_weight(end_l), total_weight(end_r)
        if length(side_l) * weight_l != length(side_r) * weight_r:
            return False
        else:
            lst = []
            if is_mobile(end_l):
                lst.append(balanced(end_l))
            if is_mobile(end_r):
                lst.append(balanced(end_r))
            if False in lst:
                return False
            else:
                return True     
