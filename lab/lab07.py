...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n == 1 or n == 2:
        ...
        return total
run_docstring_examples(num_trees, globals(), True)
#%%
def prune_leaves(t, vals):
    ...
    t_copy = copy_tree(t)
    if is_leaf(t_copy):
        if label(t_copy) in vals:
            return None
        else:
            return t_copy
    else:
        lst = []
        for b in branches(t_copy):
            lst += [prune_leaves(b, vals)]
        while None in lst:
            lst.remove(None)
        return tree(label(t_copy), lst)
run_docstring_examples(prune_leaves, globals(), True)
#%%
def dict_to_lst(d):
    ...
    for _ in range(len(d)):
        ...
        result += [pair]
    return result
run_docstring_examples(dict_to_lst, globals(), True)
# %%
